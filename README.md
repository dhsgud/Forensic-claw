...
Knowledge 백엔드는 별도 서버가 필요 없는 `Local RAG + Graph Index`(SQLite) 하나이며,
시맨틱(벡터) 검색은 선택 기능입니다. WebUI의 Knowledge 설정에서 임베딩 모델과 API Base를
지정하면, 채팅에 쓰는 동일한 OpenAI-compatible `/v1/embeddings` 엔드포인트로 임베딩을
생성해 키워드 검색과 결합합니다. 설정하지 않거나 엔드포인트에 연결할 수 없으면 자동으로
키워드 전용 검색으로 동작합니다.
...
Windows 배포 빌드는 `scripts/build-windows-msi.ps1`, `scripts/build-windows-exe.ps1`에서 관리합니다.
...
- [저장소 상태와 범위](docs/REPOSITORY_STATUS_AND_SCOPE.md)

현재 실행 기준은 이 README와 저장소 상태 문서입니다. 오래된 계획 문서는 저장소에서 제거되었습니다.
