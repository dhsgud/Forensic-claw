...
app = typer.Typer(
    ...
    help=f"{__logo__} forensic-claw - Local Forensic Assistant",
...
def _init_prompt_session() -> None:
    ...
    from forensic_claw.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} forensic-claw[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} forensic-claw[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} forensic-claw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """forensic-claw - Local Forensic Assistant."""
    pass
...
def onboard(
    ...
    if wizard_enabled:
        ...
        try:
            ...
            console.print("[yellow]Please run 'forensic-claw onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'forensic-claw agent -m "Hello!"'
    gateway_cmd = "forensic-claw gateway"
    if config:
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from forensic_claw.config.loader import load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from forensic_claw.config.paths import get_cron_dir
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show forensic-claw runtime logs during chat"),
):
...
def channels_status():
    """Show channel status."""
    from forensic_claw.channels.registry import discover_all
    from forensic_claw.config.loader import load_config
...
def _get_bridge_dir() -> Path:
    ...
    from forensic_claw.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # forensic_claw/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall forensic-claw")
        raise typer.Exit(1)
...
def channels_login(
    channel_name: str = typer.Argument(..., help="Channel name (e.g. discord, kakaotalk)"),
...
def plugins_list():
    ...
    from forensic_claw.channels.registry import discover_all, discover_channel_names
    from forensic_claw.config.loader import load_config
...
@app.command()
def status():
    """Show forensic-claw status."""
    from forensic_claw.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} forensic-claw Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from forensic_claw.providers.registry import PROVIDERS
...
def provider_login(
    ...
    """Authentication is not used in the local-only provider set."""
    from forensic_claw.providers.registry import PROVIDERS
    ...
    if not spec:
        ...
        supported = names or "(none)"
        console.print(f"[red]OAuth login is unavailable in this build.[/red]  Supported: {supported}")
        raise typer.Exit(1)

    console.print(f"[red]Login is not available for {spec.label} in this build.[/red]")
    raise typer.Exit(1)
