"""Interactive onboarding questionnaire for forensic-claw."""
...
_SELECT_FIELD_HINTS: dict[str, tuple[list[str], str]] = {
    ...
    "thinking_language": (
        ["en", "ko"],
        "Internal planning/thinking guidance language",
    ),
    "response_language": (
        ["ko", "en"],
        "Final user-facing reply language",
    ),
}
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]forensic-claw[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from forensic_claw.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"forensic_claw.channels.{name}")
